...
class Whitelabeler
{
    ...
    */
    ...
    {
        if (substr($version, 0, 3) == 2.5 || substr($version, 0, 3) == 2.6) {
        ...
        if (file_exists($app_css)) {
            ...
            $app_css_new      = str_replace(
                ...
                ['{{sidebar_background}}', '{{mautic_primary}}', '{{mautic_hover}}'],
    ...
    */
    public function replaceImages($path, $url, $version, $sidebar_image, $sidebar_width, $sidebar_margin, $login_image, $login_width, $login_margin, $favicon_image)
    {
        ...
        // convert to .ico and save.
            ...
            $ico_lib = new PHP_ICO($favicon_image['tmp_name'], [[64, 64]]);
        ...
        if (file_exists($left_panel)) {
            ...
            $left_panel_new      = str_replace(
                // Look for these template tags.
                ['{{sidebar_image}}', '{{sidebar_width}}', '{{margin_top}}', '{{margin_right}}', '{{margin_left}}'],
                // Replace template tags with values.
                [$url.'/media/images/sidebar_logo.png', $sidebar_width, $sidebar_margin['top'], $sidebar_margin['right'], $sidebar_margin['left']],
                $left_panel_template
            );
        ...
        if (file_exists($login_page)) {
            ...
            $login_page_new      = str_replace(
                // Look for these template tags.
                ['{{login_logo}}', '{{login_logo_width}}', '{{login_logo_margin_top}}', '{{login_logo_margin_bottom}}'],
                // Replace template tags with values.
                [$url.'/media/images/login_logo.png', $login_width, $login_margin['top'], $login_margin['bottom']],
                $login_page_template
            );
